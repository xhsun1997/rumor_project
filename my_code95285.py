...
def getVocabulary(input_text_data,stopWordPath=None,miniFreq=0,pretrained_word2vec_path=None,pretrained_doc2vec_path=None):
    stopWordList=[]
    ...
    if os.path.exists(pretrained_word2vec_path)==False:
        ...
        doc_model=gensim.models.doc2vec.Doc2Vec(documents=doc2vec_sentences,dm=0,vector_size=100)
        doc_model.train(documents=doc2vec_sentences,total_examples=doc_model.corpus_count,epochs=20)
        doc_model.save(pretrained_doc2vec_path)
    else:
        ...
        print("Load pretrained doc2vec model from ",pretrained_doc2vec_path)
        doc_model=gensim.models.Doc2Vec.load(pretrained_doc2vec_path)
        
    vocab, wordEmbedding = getWordEmbedding(words,model)
    word2id=dict(zip(vocab, list(range(len(vocab)))))
    return word2id,wordEmbedding,doc_model
...
def get_input_feature(input_text_data,input_social_feature_data,event_label,word2id,doc_model,
                    max_group_nums=10,max_post_nums=50,max_seq_len=30,doc_dim=100):
    ...
    input_doc_embeddings=np.zeros((num_events,max_group_nums,max_post_nums,doc_dim))
    ...
    for i,group_text_data in tqdm(enumerate(input_text_data)):
        ...
        for j,posts_text_data in enumerate(group_text_data):
            ...
            for k,text in enumerate(posts_text_data):
                ...
                text_embed=doc_model.infer_vector(tokens,alpha=0.01,steps=10)#vector
                input_doc_embeddings[i][j][k]=text_embed
    ...
    return [input_text_features,input_social_features,input_doc_embeddings]
...
def train_lstm(n_symbols,embedding_matrix,config):
    ...
    model=Model([main_input,sub_input,doc_embed_input],output_layer)
...
if __name__=='__main__':
    ...
    word2id,wordEmbedding,doc_model=getVocabulary(input_text_data,stopWordPath=stop_word_file,pretrained_word2vec_path='./word2vecModel',
                                        pretrained_doc2vec_path="./doc2vecModel")
    ...
    if os.path.exists("./input_features.pkl"):
        ...
        input_features=pickle.load(open("./input_features.pkl","rb"))
    else:
        input_features=get_input_feature(input_text_data,input_social_feature_data,event_label,word2id,doc_model,
                                                                max_group_nums=10,max_post_nums=50,max_seq_len=30)
        pickle.dump(input_features,open("./input_features.pkl",'wb'))
    input_doc_embeddings=None
    if len(input_features)==3:
        [input_text_features,input_social_features,input_doc_embeddings]=input_features
    else:
        [input_text_features,input_social_features]=input_features
    #input_doc_embeddings.shape==(num_events,max_group_nums,max_post_nums,100)
    ...
    if input_doc_embeddings is not None:
        input_doc_embeddings=input_doc_embeddings[data_index]
        train_doc_embeddings=input_doc_embeddings[:train_index]
        eval_doc_embeddings=input_doc_embeddings[train_index:(train_index+eval_index)]
        test_doc_embeddings=input_doc_embeddings[(train_index+eval_index):]
    ...
    history = model.fit([trainTextReviews,train_social_features,train_doc_embeddings],trainLabels,epochs=15,batch_size=config.batchSize,
                    validation_data=([evalTextReviews,eval_social_features,eval_doc_embeddings],evalLabels),shuffle=True,
                    callbacks=[early_stopping,reduce_lr,model_checkpoint])

    print(history.history.keys())
    predicts=model.predict([testTextReviews,test_social_features,test_doc_embeddings])#shape==(num_test_examples,2)
    y_predict_label=[]
