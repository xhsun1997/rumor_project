...
def getVocabulary(input_text_data,stopWordPath=None,miniFreq=0,pretrained_word2vec_path=None,pretrained_doc2vec_post_path=None,
    ...
    if os.path.exists(pretrained_word2vec_path)==False:
        ...
        doc_post_model=gensim.models.doc2vec.Doc2Vec(documents=doc2vec_sentences,dm=0,vector_size=100,workers=multiprocessing.cpu_count())
        ...
        doc_group_model=gensim.models.doc2vec.Doc2Vec(documents=group_doc2vec_sentences,dm=0,vector_size=200,window_size=20,workers=multiprocessing.cpu_count())
        doc_group_model.train(documents=group_doc2vec_sentences,total_examples=doc_group_model.corpus_count,epochs=30)
...
def get_input_feature(input_text_data,input_social_feature_data,event_label,word2id,doc_post_model,doc_group_model,
                    max_group_nums=10,max_post_nums=50,max_seq_len=30,doc_dim=100,group_doc_dim=200):
...
if __name__=='__main__':
    ...
    input_post_embeddings=None
    input_group_embeddings=None
    ...
    if input_post_embeddings is not None:
        input_post_embeddings=input_post_embeddings[data_index]
        train_doc_embeddings=input_post_embeddings[:train_index]
        eval_doc_embeddings=input_post_embeddings[train_index:(train_index+eval_index)]
        test_doc_embeddings=input_post_embeddings[(train_index+eval_index):]
    ...
    if input_group_embeddings is not None:
        input_group_embeddings=input_group_embeddings[data_index]
        train_group_doc_embeddings=input_group_embeddings[:train_index]
        eval_group_doc_embeddings=input_group_embeddings[train_index:(train_index+eval_index)]
        test_group_doc_embeddings=input_group_embeddings[(train_index+eval_index):]
    ...
    model.load_weights("./save_dir/model_13-1.00.hdf5")
    model.summary()
